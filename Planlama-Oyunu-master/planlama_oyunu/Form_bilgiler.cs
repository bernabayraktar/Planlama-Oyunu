    ...
    public partial class Form_bilgiler : Form
    {
        ...
        private void listele()
        {
            ...
            while (reader.Read())
            {
                ...
                lst.SubItems.Add(reader["alim_tarihi"].ToString());
                lst.SubItems.Add(reader["ekleme_tarihi"].ToString());
                listwiev_item_envanter.Items.Add(lst);
            }
            while (oku.Read())
            {
                txtad.Text = oku["ad"].ToString();
                txtsoyad.Text = oku["soyad"].ToString();
                txtkullanici.Text = oku["kullanıcı"].ToString();
                txtpassword.Text = oku["şifre"].ToString();
        ...
        void gridDoldur()
        {
            ...
            command.Parameters.AddWithValue("@itemad", cmb_selected_item.SelectedItem);
        ...
        {
            TXT_USERID_TXT.Text = Form_login.usertype.ToString();
            gridDoldur();
            listele();
        }
        ...
        {
            k.kullanıcı_güncelle(TXT_USERID_TXT.Text, txtad.Text ,txtsoyad.Text ,txtkullanici.Text ,txtpassword.Text ,txttc.Text ,txttelefon.Text ,txtemail.Text ,txtadres.Text );
            listele();
        ...
        private void btn_para_başvuru_Click(object sender, EventArgs e)
        {
            //money_type = cmb_money_type.Text;
            if (cmb_money_type.Text == "" || txt_para_başvuru.Text == "") 
            {
                MessageBox.Show("Lütfen bir para birimi veya para miktarı giriniz", "ERROR", MessageBoxButtons.OK, MessageBoxIcon.Error);
            }
            else
            {
                para.para_başvuru(Form_login.usertype, Convert.ToDouble(txt_para_başvuru.Text), cmb_money_type.Text);

...
namespace planlama_oyunu
{
    class para
    {
        OleDbConnection connection = new OleDbConnection("Provider = Microsoft.Jet.OLEDB.4.0; Data Source = planlama oyunu db.mdb");
        ...
        public void para_başvuru(int id,double para_miktar,string birim)
        {
            ...
            connection.Open();
            OleDbCommand komut = new OleDbCommand("insert into para_başvuru (userID,para_miktar,para_birim) values('" + this.para_userID + "','" + this.para_miktar + "','"+this.para_birim+"')", connection);
